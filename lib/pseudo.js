...
export default class PrefixTree {
  constructor() {
    this.root = new Node(''); // Passes an empty string into the new Node('') it starts here from the root case, and the first letter becomes the roots child
  ...
  insert (word) {
    word = word.toLowerCase().split(''); // we take the word and split it into an array of strings of the letters in the word ** Example pass in 'apple' and => ['a', 'p', 'p', 'l', 'e']
   
    let Parent = this.root; // we are assigning Parent to this.root
    let position = this.root.children;

    for (let i = 0; i < word.length; i++) {// we are using forEach() to iterate over each letter of the word that was passed after it was word.split()
      if (!position.hasOwnProperty(word[i])) { // if the letter does not exist in the chidren object
        position[word[i]] = new Node(word[i]); // create a new Node using that letter
      }
      position = position[word[i]].children;
      parent = parent.children[word[i]]; // assigning currentNode to the currentNode's child to keep moving down the Trie
    }
    if (!parent.wordEnd) { // if the word is already in the Trie
      this.wordCount++; // increment the word count **this does not happen if it is not a word**
      }
    parent.wordEnd = true;  // if the conditions are met, the new word is added to the Trie and the wordEnd is noted/marked to prevent duplicates
  }
 ...
 // ***** on the second iteration currentNode = carNode and the newPhrase = 'car' *****
    ...
    return suggestions.map(newWord => { // it is mapping over the suggestion array (which is an array of objects{}) and the objects that contain a key value pair of word: 'whatever word we are on', popCount: 'it's popularity count
      return wordObj.word; // Only returing the word, from the newWord
